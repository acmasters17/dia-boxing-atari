...
class MoreSwayAggressiveJugglingReactiveAgent(Agent):
    ...
    def juggleBetweenFists(self):
        ...
        if(self.lastAction == BoxingAction.FIRE_MOVE_UP and self.midCombo == False):
            self.midCombo = True
            return BoxingAction.NOOP
        elif(self.lastAction == BoxingAction.NOOP and self.midCombo == True):
            return BoxingAction.FIRE_MOVE_UP
        elif(self.lastAction == BoxingAction.FIRE_MOVE_UP and self.midCombo == True):
            return BoxingAction.MOVE_DOWN
        elif(self.lastAction == BoxingAction.MOVE_DOWN):
